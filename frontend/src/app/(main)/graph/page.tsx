...
function GraphNode({ data }: NodeProps) {
  ...
  return (
    ...
    >
      <span
        className="flex items-center justify-center w-5 h-5 rounded-md text-[11px] font-bold flex-shrink-0"
  ...
  return (
    <div className="h-[calc(100vh-3rem)] flex flex-col">
      <div className="flex items-center justify-between mb-4 gap-4 flex-wrap">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight">Attack Surface Graph</h1>
        ...
        <div className="flex items-center gap-3 flex-wrap">
          <div className="relative">
            <select
              ...
              className="h-9 pl-3 pr-8 rounded-md bg-secondary/60 border border-border text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-primary/40 appearance-none cursor-pointer"
            >
          ...
          {!bubbleMode && (
            <div className="flex p-1 gap-1 bg-secondary/60 border border-border rounded-md">
          ...
          {bubbleMode && (
            <div className="flex items-center gap-1.5 px-3 py-1.5 bg-secondary/60 border border-border rounded-md text-xs text-muted-foreground">
      ...
      <div className="flex-1 flex gap-4 min-h-0">
        ...
        {bubbleMode ? (
          ...
          <div className="w-72 glass-card rounded-md p-4 overflow-y-auto">
            <h3 className="font-semibold text-sm mb-3">Node Detail</h3>
            <div className="space-y-2 text-sm">
              <div>
                <span className="text-xs uppercase tracking-wider text-muted-foreground block">Label</span>
                <p className="font-medium">{selectedNode.label}</p>
              </div>
              <div>
                <span className="text-xs uppercase tracking-wider text-muted-foreground block">ID</span>
                <p className="font-mono text-xs truncate">{selectedNode.id}</p>
              </div>
              {selectedNode.info && (
                <div>
                  <span className="text-xs uppercase tracking-wider text-muted-foreground block">Details</span>
                  <div
...
function StatBadge({ label, count }: { label: string; count: number }) {
  return (
    <div className="flex items-center gap-1.5 px-2.5 py-1.5 bg-secondary/60 border border-border rounded-md">
      <span className="tech text-sm font-medium leading-none">{count}</span>
...
function Legend() {
  return (
    <div className="w-56 bg-card border rounded-md p-4 space-y-3 overflow-y-auto">
      ...
      {Object.entries(GROUP_META).map(([group, meta]) => (
        <div key={group} className="flex items-center gap-2 text-sm">
          <span
            className="w-4 h-4 rounded flex items-center justify-center text-[11px] font-bold flex-shrink-0"
        ...
        {groups.map((g) => {
          ...
          return (
            ...
            >
              ...
              >
                <span className="tech text-2xl leading-none drop-shadow">{g.count}</span>
                <span className="text-[11px] uppercase tracking-wider opacity-90">bugs</span>
              </span>
...
function BubbleDetailPane({
  ...
  if (group) {
    return (
      <div className="w-80 flex flex-col glass-card rounded-md overflow-hidden">
        <div className="p-4 border-b border-border/60">
          ...
          <p className="text-[13px] text-muted-foreground mt-1.5">
            <span className="tech">{group.count}</span> occurrence{group.count > 1 ? "s" : ""} ·{" "}
            {group.tools.length ? `via ${group.tools.join(", ")}` : "various tools"}
          </p>
        </div>
        <div className="flex-1 overflow-y-auto p-3 space-y-2">
          {group.items.map((f, i) => (
            <details key={f.id || i} className="rounded-md border border-border/70 bg-card/60 group/details">
              ...
              <div className="px-3 pb-3 pt-1 text-xs space-y-2">
                ...
                {f.remediation && (
                  <div>
                    <p className="text-[11px] uppercase tracking-wider text-muted-foreground mb-0.5">Remediation</p>
  ...
  return (
    <div className="w-80 glass-card rounded-md p-4 overflow-y-auto">
      ...
      {totalGroups === 0 ? (
        ...
        <div className="space-y-2">
          <div className="text-sm">
            <span className="tech font-medium">{summary.total}</span>{" "}
          ...
          {Object.keys(SEVERITY_COLORS).map((sev) => {
            ...
            return (
              <div key={sev} className="flex items-center gap-2 text-xs">
                ...
                <span className="tech font-medium">{n}</span>
              </div>
            );
          })}
...
function Badge({ children, color, pill }: { children: React.ReactNode; color: string; pill?: boolean }) {
  return (
    <span
      className={`px-2 py-0.5 text-[11px] font-semibold ${pill ? "rounded-full" : "rounded-md"} flex-shrink-0`}
