Web message manipulation | Web Security Academy

- Web Security Academy
- DOM-based
- Web message manipulation Web message manipulation

# 

 In this section, we'll explain what web-message manipulation vulnerabilities are and suggest ways to reduce your exposure to them. What is DOM-based web message manipulation?
...
## 
 ...
 The
```
 postMessage() method for sending web messages can lead to vulnerabilities if the event listener for receiving messages handles the incoming data in an unsafe way. How to prevent DOM-based web message manipulation

## 

 In addition to the general measures described on the DOM-based vulnerabilities page, you should avoid sending web messages that contain data that originated from any untrusted source. When sending cross-origin messages, you should always explicitly specify the target window. Most importantly, make sure to implement robust measures to verify the origin of any incoming messages. Find DOM-based vulnerabilities using Burp Suite
